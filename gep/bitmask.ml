...
let void_mask = BITMASK(Bytes.empty)
...
let bytes_map2 f s1 s2 =
  let l = Bytes.length s1 in
  let l2 = Bytes.length s2 in
  let res = Bytes.create l in
  let rec aux n =
    if n < l then
      (Bytes.set res n (f (Bytes.get s1 n) (Bytes.get s2 n));
       aux (n + 1))
    else
      ()
  in
...
let bit_count m =
  ...
  in
  Bytes.to_seq v |> Seq.fold_left aux 0


(** returns the length in bit of a mask *)
let length m =
  Bytes.length (get_intern_val m)
...
let sub m pos length =
  ...
  try
    ...
    BITMASK(Bytes.sub v pos length)
  with
...
let set_same_length m1 m2 left_or_right =
  ...
  let l1 = Bytes.length v1 in
  let l2 = Bytes.length v2 in
  let l_ext = abs (l1 - l2) in
  let v_ext = Bytes.make l_ext '0' in
  if l1 == l2 then
    (m1, m2)
  else if l1 < l2 then
    (BITMASK(if left_or_right then Bytes.concat v_ext [v1] else Bytes.concat v1 [v_ext]), m2)
  else
    (* l1 > l2 *)
    (m1, BITMASK(if left_or_right then Bytes.concat v_ext [v2] else Bytes.concat v2 [v_ext]))
...
let logand m1 m2 =
  ...
  if (Bytes.length v1) != (Bytes.length v2) then
    ...
    BITMASK(bytes_map2 bit_and v1 v2)
...
let logor m1 m2 =
  ...
  if (Bytes.length v1) != (Bytes.length v2) then
    ...
    BITMASK(bytes_map2 bit_or v1 v2)
...
let shift_left m sh =
  ...
  if sh < 0 then
    ...
    BITMASK(Bytes.concat s [Bytes.make sh '0'])
...
let shift_right m sh =
  ...
  if sh < 0 then
    ...
    (if l == 0 || sh >= l then
       ...
       (let s_ext = Bytes.make sh (Bytes.get s 0) in
        BITMASK(Bytes.concat s_ext [Bytes.sub s 0 (l - sh)])))
...
let masked_value m mask =
  ...
  let l = min (Bytes.length v) (Bytes.length vmask) in
  let lr = min l (bit_count mask) in
  let res = Bytes.create lr in
  let set_res n n_res =
    if (Bytes.get v n) <> '0' && (Bytes.get v n) <> '1' then
      failwith "shouldn't happen (bitmask.ml::masked_value)"
    else
      Bytes.set res n_res (Bytes.get v n)
  in
  let rec aux step step_res =
    if step >= l then
      ()
    else
      (if (Bytes.get vmask step) == '1' then
  ...
  (*Printf.printf "masked_value, v=%s, vmask=%s, l=%d, lr=%d\n" v vmask l lr;
    ...
    v (Bytes.length v) vmask (Bytes.length vmask) res (Bytes.length res);*)
  BITMASK(res)
...
let unmask m mask =
  ...
  let lv = Bytes.length v in
  let l = min lv (Bytes.length vm) in
  let res = Bytes.create lv in
  let v_suffix = Bytes.sub v l (lv - l) in
  let set_res n c =
    if c <> '0' && c <> '1' then
  ...
  let rec aux step =
    if step >= l then
      ...
      (if (Bytes.get vm step) == '1' then
         set_res step '0'
       else
         set_res step (Bytes.get v step);
       aux (step + 1)
      )
  in
  aux 0;
  (*!!DEBUG!!*)
  (*Printf.printf "unmask, m= [%s](%d), mask= [%s](%d), res= [%s](%d)\n" v lv vm (Bytes.length vm) (res^v_suffix) (Bytes.length (res^v_suffix));*)
  BITMASK(Bytes.concat res [v_suffix])
...
let is_null m =
  let s = get_intern_val m in
  try
    Bytes.iter (fun x -> if x <> '0' then raise Not_null_mask) s;
    true
  with
  | Not_null_mask -> false
...
let to_int32 m =
  ...
  if size > 32 then
    ...
    try
      "0b" ^ (Bytes.unsafe_to_string s)|> Int32.of_string
    with
...
let of_int i =
  ...
  (*!!DEBUG!!*)
  (*let res =*)
  BITMASK(string_hex_to_bin si |> Bytes.of_string)
...
let to_int32_list m =
  ...
  let rec aux accu step =
    ...
    | n when n >= 32 -> aux (accu @ [(Int32.of_string ("0b" ^ Bytes.unsafe_to_string (Bytes.sub s step 32)))]) (step + 32)
    | n when n > 0 && n < 32 -> accu @ [Int32.shift_left (Int32.of_string ("0b" ^ Bytes.unsafe_to_string (Bytes.sub s step n))) (32 - n)] (* less than 32 bits from end of string *)
...
let mask_range n m =
  ...
  BITMASK(Bytes.concat (Bytes.make (n - m + 1) '1') [(Bytes.make m '0')])
    ...
    | h::t ->
      (match h with
       | Str.Text(txt) ->
         ...
         Bytes.concat (transform_str txt) [(get_mask_from_regexp_list t)]
       | Str.Delim(d) ->
         Bytes.concat (Bytes.make (get_length_from_format d) '0') [(get_mask_from_regexp_list t)]
      )
  in
  ...
  let revert_bytes s =
    let rec aux length str =
      if length < 8 then
        ...
        Bytes.concat (aux (length - 8) (Bytes.sub str 8 (length - 8))) [(Bytes.sub str 0 8)]
    in
    if get_bit_image_order () then
      aux (Bytes.length s) s
    else
      s
  in
  (* !!DEBUG!! *)
  (*let res =*)
  BITMASK(get_mask_from_regexp_list (Str.full_split (Str.regexp "%[0-9]*[bdfxs]") (remove_space (get_str (get_expr_from_iter_value (Iter.get_attr sp "image"))))) |> revert_bytes)
(*in
...
let get_value_mask sp =
  ...
  (*!DEBUG!!*)
  (*let res =*)
  BITMASK(Bytes.unsafe_of_string
            (get_mask_from_regexp_list
               (Str.full_split (Str.regexp "%[0-9]*[bdfxs]")
                  (remove_space (get_str (get_expr_from_iter_value (Iter.get_attr sp "image"))))))|>revert_bytes)
(*in
  let s = get_intern_val res in
...
let get_value sp =
  ...
  (*!DEBUG!!*)
  (*let res =*)
  BITMASK(Str.global_replace (Str.regexp "X+") "" (Bytes.unsafe_to_string vm) |> Bytes.unsafe_of_string)
(*in
  let s = get_intern_val res in
...
let to_string = function BITMASK m -> Bytes.unsafe_to_string m


(** Print the mask.
    @param m	Mask to print. *)
let print m = to_string m |> print_string
